...
struct hash_table_v1 *hash_table_v1_create() {
    ...
    int e = pthread_mutex_init(&hash_table->lock, NULL);
    if (e != 0) {
        exit(e);
    }
...
void hash_table_v1_add_entry(struct hash_table_v1 *hash_table,
                             const char *key,
                             uint32_t value) {
    int e = pthread_mutex_lock(&hash_table->lock);
    if (e != 0) {
        exit(e);
    }
    struct hash_table_entry *hash_table_entry = get_hash_table_entry(hash_table, key);
    ...
    if (list_entry != NULL) {
        ...
        e = pthread_mutex_unlock(&hash_table->lock);
        if(e != 0){
            exit(e);
        }
        return;
...
void hash_table_v1_destroy(struct hash_table_v1 *hash_table) {
    ...
    int e = pthread_mutex_destroy(&hash_table->lock);
    if (e != 0) {
        exit(e);
    }
    free(hash_table);
