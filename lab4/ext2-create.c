...
void set_bit(u8 *bit_map, u32 number)
{
    u32 bit_num = (number - 1) % 8;
	u32 byte_num = (number - 1) / 8;
    bit_map[byte_num] |= (1 << bit_num);
}
...
void write_block_bitmap(int fd) {
	...
	for (u32 i = 1; i < LAST_BLOCK + 1; i++) set_bit(map_value, i);

    for (u32 i = NUM_INODES * 8; i <= NUM_BLOCKS * 8; i++) set_bit(map_value, i);
...
void write_inode_bitmap(int fd)
{
	...
	for(u32 i=1; i<LAST_INO+1; i++) set_bit(map_value,i);

	for(u32 i=NUM_INODES+1; i<=NUM_BLOCKS*8; i++) set_bit(map_value,i);
